...
Aplikace běží na vlastním VPS serveru Hetzner za Nginx reverse proxy.

```bash
# Na serveru: aktualizace aplikace
git pull
source venv/bin/activate
pip install -r requirements.txt
python manage.py migrate
python manage.py collectstatic --noinput
sudo systemctl restart spotrebnik
```

Proměnné prostředí nastavit v `/etc/systemd/system/spotrebnik.service` nebo v `.env`:
```
SECRET_KEY=<hodnota>
DEBUG=False
DATABASE_URL=<postgres-url>
ALLOWED_HOSTS=<domena>
```
