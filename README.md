...
Aplikace běží na Render.com — deploy se spustí automaticky při každém push na `main`.

Procfile (automaticky spouštěný Renderem):
```
web: gunicorn spotrebnik.wsgi
```

Proměnné prostředí nastavit v Render Dashboard → Environment:
```
SECRET_KEY=<hodnota>
DEBUG=False
DATABASE_URL=<postgres-url z Render PostgreSQL>
ALLOWED_HOSTS=<render-domena>.onrender.com
```

Databáze: Render PostgreSQL (připojení přes `DATABASE_URL`).
